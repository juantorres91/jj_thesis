...
def MolGen(SFIL):
    ...
    m.N= Var( m.MoleL, domain = NonNegativeIntegers, initialize=1)     # Molecule length 
    ...
    m.N['n'].setlb(m.nmin.value)
    m.N['m'].setlb(m.mmin.value)

    m.N['n'].setub(m.nmax.value)
    m.N['m'].setub(m.mmax.value)
    m.N['k'].fix(0)
...
def MoleIni(SFIL):
    ...
    m.obj=Objective(expr=- (m.y_prop['STR']))

    # HLB constraint
    def hlb_rule(m,j):
        if j == 'HLB': 
            return (12, m.y_prop[j], 16)
        elif j =='CP':
            return (30, m.y_prop[j],  90)
        else:
            return Constraint.Skip
        
    m.hlb_cons = Constraint(m.MoleP, rule = hlb_rule)
